   ...
   subroutine share(this,nover)
      ...
      if (present(nover)) then
         ...
         if (no.gt.this%cfg%no) then
            call die('[dpd_class share] Specified overlap is larger than that of cfg - reducing no')
            no=this%cfg%no
         else if (no.le.0) then
            call die('[dpd_class share] Specified overlap cannot be less or equal to zero')
         else if (this%cfg%nx.gt.1.and.no.gt.this%cfg%nx_) then
            call die('[dpd_class share] Specified overlap spans multiple procs in x')
         else if (this%cfg%ny.gt.1.and.no.gt.this%cfg%ny_) then
            call die('[dpd_class share] Specified overlap spans multiple procs in y')
         else if (this%cfg%nz.gt.1.and.no.gt.this%cfg%nz_) then
            call die('[dpd_class share] Specified overlap spans multiple procs in z')
         end if
      else
         no=1
      end if
