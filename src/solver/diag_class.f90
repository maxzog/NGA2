...
=======
   ...
   !> diag object definition
   type :: diag
      ! Diagonal solver works for a config
      type(config), pointer :: cfg                                    !< Config for the diag solver
      character(len=str_medium) :: name                               !< Name of solver
      integer :: ndiags                                               !< Number of diagonals
      real(WP), dimension(:,:,:,:), allocatable :: Ax,Ay,Az           !< Working arrays
...
contains
   ...
   subroutine linsol_x(this)
      ...
      case(5)
         ...
         this%Rx,this%cfg%nx_,this%cfg%ny_*this%cfg%nz_,'x',this%stackmem(1,1),this%stackmem(1,3))
      case default
         call this%polydiagonal((this%ndiags-1)/2,&
         this%Ax(this%cfg%jmin_,this%cfg%kmin_,this%cfg%imin_,-(this%ndiags-1)/2),&
         this%Rx,this%cfg%nx_,this%cfg%ny_*this%cfg%nz_,'x',this%stackmem)
      end select
   ...
   subroutine linsol_y(this)
      ...
      case default
         call this%polydiagonal((this%ndiags-1)/2,&
         this%Ay(this%cfg%imin_,this%cfg%kmin_,this%cfg%jmin_,-(this%ndiags-1)/2),&
         this%Ry,this%cfg%ny_,this%cfg%nx_*this%cfg%nz_,'y',this%stackmem)
      end select
   ...
   subroutine linsol_z(this)
      ...
      case default
         call this%polydiagonal((this%ndiags-1)/2,&
         this%Az(this%cfg%imin_,this%cfg%jmin_,this%cfg%kmin_,-(this%ndiags-1)/2),&
         this%Rz,this%cfg%nz_,this%cfg%ny_*this%cfg%nx_,'z',this%stackmem)
      end select
   ...
   subroutine tridiagonal(this,A,B,C,R,n,lot,dir,s1,s2)
      ...
      if (.not.nper) then
         ...
         buf5=recvbuf1(:,5,:)
         buf6=recvbuf1(:,6,:)
         call pentadiagonal_periodic_serial(buf1,buf2,buf3,buf4,buf5,buf6,2*proc,nlot,sendbuf(1),sendbuf(2*proc*nlot+1))
         recvbuf1(:,1,:)=buf1
         recvbuf1(:,2,:)=buf2
         recvbuf1(:,3,:)=buf3
         recvbuf1(:,4,:)=buf4
