...
contains
   ...
   subroutine advance(this,dt,U,V,W,rho,visc,use_crw,sgs)
      use mathtools, only: Pi
      use random,    only: random_normal
      use sgsmodel_class, only: sgsmodel, get_sgstke
      implicit none
      class(lpt), intent(inout) :: this
      real(WP), intent(inout) :: dt  !< Timestep size over which to advance
      ...
      real(WP) :: dW1,dW2,dW3,tmp1,tmp2,tmp3
      logical, intent(in) :: use_crw
      integer :: i
      real(WP) :: mydt,dt_done
      real(WP) ::  sig_sg,tau_crwi
      ...
      do i=1,this%np_
         ...
         do while (dt_done.lt.dt)
            ...
            call this%get_rhs(U=U,V=V,W=W,rho=rho,visc=visc,p=this%p(i),acc=acc,opt_dt=this%p(i)%dt,use_crw=use_crw)
            ...
            if (use_crw) then
               ...
               jj = this%p(i)%ind(2)
               kk = this%p(i)%ind(3)
               tke = get_sgstke(visc=sgs%visc(ii,jj,kk), delta=sgs%delta(ii,jj,kk), rho=rho(ii,jj,kk))
               
               sig_sg = sqrt(0.6666_WP*tke)
               tau_crwi = sig_sg/sgs%delta(ii,jj,kk)/C_poz

               this%p(i)%a_crw = tau_crwi
               this%p(i)%b_crw = sig_sg*sqrt(2.0_WP*tau_crwi)

               dW1 = random_normal(m=0.0_WP,sd=sqrt(mydt))
               dW2 = random_normal(m=0.0_WP,sd=sqrt(mydt))
               dW3 = random_normal(m=0.0_WP,sd=sqrt(mydt))
               
               tmp1 = (1.0_WP - this%p(i)%a_crw*mydt)*this%p(i)%uf(1) + this%p(i)%b_crw*dW1 - &
                       this%p(i)%b_crw*0.5_WP*(mydt + epsilon(1.0_WP))**(-0.5_WP)*(dW1**2 - mydt) 
               tmp2 = (1.0_WP - this%p(i)%a_crw*mydt)*this%p(i)%uf(2) + this%p(i)%b_crw*dW2 - &
                       this%p(i)%b_crw*0.5_WP*(mydt + epsilon(1.0_WP))**(-0.5_WP)*(dW2**2 - mydt) 
               tmp3 = (1.0_WP - this%p(i)%a_crw*mydt)*this%p(i)%uf(3) + this%p(i)%b_crw*dW3 - &
                       this%p(i)%b_crw*0.5_WP*(mydt + epsilon(1.0_WP))**(-0.5_WP)*(dW3**2 - mydt)
            end if
   ...
   subroutine get_rhs(this,U,V,W,rho,visc,T,p,acc,opt_dt,use_crw)
      implicit none
      class(lpt), intent(inout) :: this
      real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(inout) :: U     !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
      real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(inout) :: V     !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
