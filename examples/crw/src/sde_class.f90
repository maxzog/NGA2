...
!  real(WP), parameter, public :: tau_crwi = 0.5702_WP
  ...
  real(WP), parameter, public :: Rc = 0.1_WP
  ...
  type :: part
     ...
     integer  :: update                   !< Update eddy velocity (0=no, 1=yes)? (DRW)
  end type part
  !> Number of blocks, block length, and block types in a particle
  integer, parameter                         :: part_nblock=3
  integer           , dimension(part_nblock) :: part_lblock=[1,15,5]
...
contains
  ...
  subroutine get_diffusion(this, eddyvisc, rho, SR, Cs_arr, U, V, W)
   ...
   real(WP) :: fvisc,frho,fsr,fCs,delta,sig_sgs,eps_sgs,b_crw,tke_sgs,tau_crwi
   ...
   do i=1,this%np_ 
      ...
      if (fvisc.gt.0.2_WP) fvisc = 0.0_WP
      ...
      Tcr = -taup * log(1.0_WP - (Le/(taup*norm2(fld - this%p(i)%vel))))
      
      this%p(i)%teddy = min(2.0_WP * Tl, Tcr) 
      
      b_crw   = sig_sgs !sqrt(1.3333_WP*tke_sgs*tau_crwi)  ! Diffusion coefficient
  ...
  subroutine get_drift(this,eddyvisc,rho,SR,p,drift,Cs_arr)
   ...
   real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(inout) :: SR            !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
   real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(inout) :: Cs_arr        !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
   real(WP) :: fvisc,frho,fsr,fCs,delta,sig_sgs,eps_sgs,tke_sgs,tau_crwi
   real(WP) :: C,Cy
...
!             tmp3 = (-dot_product(this%p(i)%us(:),guz) + tauz)*mydt + (1.0_WP - a_crw*mydt)*this%p(i)%us(3) + b_ij(3)/sqrt(corrsum)
           ...
           !  tmp3 = (1.0_WP - a_crw*mydt)*this%p(i)%us(3) + this%p(i)%dW(3)*rmydt
             ...
             tmp1 = this%p(i)%dW(1)!*rmydt
             tmp2 = this%p(i)%dW(2)!*rmydt
             tmp3 = this%p(i)%dW(3)!*rmydt
          end if
