...
module simulation
   ...
   !> Particles
   integer :: ngroups
   integer :: np_per_group,np
   real(WP), dimension(:), allocatable :: group_diameters
...
!      end block update_timetracker
      ...
      initialize_lpt: block
         ...
         ! Get number of particle groups
         ngroups=param_getsize('Particle diameters')
         ! Allocate diameter array
         allocate(group_diameters(1:ngroups))
         ! Get particle diameters
         call param_read('Particle diameters',group_diameters)
         ! Get number of particles per group
         call param_read('Particles per group',np_per_group)
         ! Compute total number of particles
         np=ngroups*np_per_group
         ...
         if (lp%cfg%amRoot) then
            ...
            do i=1,np
               ...
               if (mod(i,np_per_group).eq.1) group=group+1
               ! Get diameter
               dp=group_diameters(group)
               ! Set the diameter
               lp%p(i)%d=dp
