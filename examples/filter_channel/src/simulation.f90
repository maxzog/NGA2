...
module simulation
   ...
   real(WP), dimension(:,:,:), allocatable :: Uif,Vif,Wif
   real(WP), dimension(:,:,:), allocatable :: Uis,Vis,Wis
...
!      counter_=0.0_WP
   ...
   subroutine simulation_init
      ...
      allocate_work_arrays: block
         ...
         allocate(Uif (cfg%imino_:cfg%imaxo_,cfg%jmino_:cfg%jmaxo_,cfg%kmino_:cfg%kmaxo_)); Uif =0.0_WP 
         allocate(Vif (cfg%imino_:cfg%imaxo_,cfg%jmino_:cfg%jmaxo_,cfg%kmino_:cfg%kmaxo_)); Vif =0.0_WP 
         allocate(Wif (cfg%imino_:cfg%imaxo_,cfg%jmino_:cfg%jmaxo_,cfg%kmino_:cfg%kmaxo_)); Wif =0.0_WP 
         allocate(Uis (cfg%imino_:cfg%imaxo_,cfg%jmino_:cfg%jmaxo_,cfg%kmino_:cfg%kmaxo_)); Uis =0.0_WP 
         allocate(Vis (cfg%imino_:cfg%imaxo_,cfg%jmino_:cfg%jmaxo_,cfg%kmino_:cfg%kmaxo_)); Vis =0.0_WP 
         allocate(Wis (cfg%imino_:cfg%imaxo_,cfg%jmino_:cfg%jmaxo_,cfg%kmino_:cfg%kmaxo_)); Wis =0.0_WP 
...
!      end block update_sgs
      ...
      create_ensight: block
         ...
         call ens_out%add_vector('velocity_filter',Uif,Vif,Wif)
         call ens_out%add_vector('velocity_subgrid',Uis,Vis,Wis)
...
!         end block subgrid
         ...
         filter_velocity: block
            ...
            ! Filter the cell-centered velocity field
            do k=fs%cfg%kmin_,fs%cfg%kmax_
               do j=fs%cfg%jmin_,fs%cfg%jmax_
                  do i=fs%cfg%imin_,fs%cfg%imax_
                     Uif(i,j,k) = sum(sgs%filterd(:,:,:,i,j,k) * Ui(i-1:i+1,j-1:j+1,k-1:k+1))
                     Vif(i,j,k) = sum(sgs%filterd(:,:,:,i,j,k) * Vi(i-1:i+1,j-1:j+1,k-1:k+1))
                     Wif(i,j,k) = sum(sgs%filterd(:,:,:,i,j,k) * Wi(i-1:i+1,j-1:j+1,k-1:k+1))
                  end do
               end do
            end do

            ! call MPI_ALLREDUCE(MPI_IN_PLACE,Uif,fs%cfg%nx*fs%cfg%ny*fs%cfg%nz,MPI_REAL_WP,MPI_SUM,fs%cfg%comm,ierr)
            ! call MPI_ALLREDUCE(MPI_IN_PLACE,Vif,fs%cfg%nx*fs%cfg%ny*fs%cfg%nz,MPI_REAL_WP,MPI_SUM,fs%cfg%comm,ierr)
            ! call MPI_ALLREDUCE(MPI_IN_PLACE,Wif,fs%cfg%nx*fs%cfg%ny*fs%cfg%nz,MPI_REAL_WP,MPI_SUM,fs%cfg%comm,ierr)

            ! Get the subfilter velocity field
            Uis = Ui - Uif
            Vis = Vi - Vif
            Wis = Wi - Wif
            ...
            call fs%cfg%integrate(A=Uis,integral=meanUs); meanUs=meanUs/fs%cfg%vol_total
            call fs%cfg%integrate(A=Vis,integral=meanVs); meanVs=meanVs/fs%cfg%vol_total
            call fs%cfg%integrate(A=Wis,integral=meanWs); meanWs=meanWs/fs%cfg%vol_total

            ! Get a-priori subfilter Reynolds stress 
            do k=fs%cfg%kmin_,fs%cfg%kmax_
               do j=fs%cfg%jmin_,fs%cfg%jmax_
                  do i=fs%cfg%imin_,fs%cfg%imax_
                     uiuj(1,i,j,k) = (Uis(i,j,k)-meanUs)*(Uis(i,j,k)-meanUs) 
                     uiuj(2,i,j,k) = (Vis(i,j,k)-meanVs)*(Vis(i,j,k)-meanVs) 
                     uiuj(3,i,j,k) = (Wis(i,j,k)-meanWs)*(Wis(i,j,k)-meanWs) 
                     uiuj(4,i,j,k) = (Uis(i,j,k)-meanUs)*(Vis(i,j,k)-meanVs) 
                     uiuj(5,i,j,k) = (Vis(i,j,k)-meanVs)*(Wis(i,j,k)-meanWs) 
                     uiuj(6,i,j,k) = (Uis(i,j,k)-meanUs)*(Wis(i,j,k)-meanWs) 
                  end do
               end do
            end do
         end block filter_velocity 
