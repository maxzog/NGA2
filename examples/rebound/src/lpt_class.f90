...
module lpt_class
  ...
  type :: part
     ...
     real(WP) :: delta_t    !< Collision overlap
  ...
  integer           , dimension(part_nblock) :: part_lblock=[11,28,4]
  ...
  type :: lpt
   ...
   contains
     ...
     procedure :: collide_marshall                       !< Evaluate interparticle collision force
     procedure :: collide_bons                           !< Evaluate interparticle collision force
...
contains
      ...
      !   r_influ=0.0_WP
        ...
        if (delta_n.gt.0.0_WP) then
         ...
         !   end if
           ...
           eta_n=m1*eta_coeff_w
           eta_n=0.0_WP
           k_n=real(1.72E+07,WP)
           f_n=-k_n*delta_n*n12-eta_n*rnv*n12
           ! Tangential collision
           f_t=0.0_WP
           if (this%mu_f.gt.0.0_WP) then
              k_t=real(1.48E+07,WP)
              eta_t=eta_n
              ...
              this%p(i1)%delta_t=this%p(i1)%delta_t + dot_product(t12,t12/rtv)*dt    ! Tangential contact displacement
              if (rnv*dt/d1.gt.aclipnorm) then
                 if (rtv/rnv.lt.rcliptan) rtv=0.0_WP
              else
                 if (rtv*dt/d1.lt.acliptan) rtv=0.0_WP
              ...
              if (rtv.gt.0.0_WP) then
                  ...
                  Ff_damp=-k_t*this%p(i1)%delta_t - eta_t*rtv
                  ! Sliding tangential contact
                  Ff_slip=-this%mu_f*abs(-k_n*delta_n - eta_n*rnv)
                  ! Prevent tangential displacement from growing during slip
                  if (abs(Ff_damp).gt.abs(Ff_slip)) then
                     this%p(i1)%delta_t=this%p(i1)%delta_t - dot_product(t12,t12/rtv)*dt          
                  end if
                  ! Recompute elastic & damping contact force w/ corrected displacement
                  Ff_damp=-k_t*this%p(i1)%delta_t - eta_t*rtv
                  ! Return the smaller of the two forces ( Ff_slip if Ff_damp > Ff_slip ) 
                  f_t=-min(abs(Ff_damp), abs(Ff_slip))*t12/rtv
              end if
              ! Debug stuff
            !   if (this%p(i1)%delta_t(1).gt.this%p(i1)%pos0(1)) this%p(i1)%pos0(1)=this%p(i1)%delta_t(1)
            !   if (abs(f_n(2))/delta_n.gt.this%p(i1)%pos0(1)) this%p(i1)%pos0(1)=abs(f_n(2))/delta_n
           end if
           ! Calculate collision force
           f_n=f_n/m1; f_t=f_t/m1
           this%p(i1)%Acol=this%p(i1)%Acol+f_n+f_t
   ...
   collision_force: block
     ...
     collision: do i1=1,this%np_
        ...
        if (isOverlap.or.isCol) then
           ...
           ! calculate collision torque
           this%p(i1)%tcol=this%p(i1)%tcol+cross_product(0.5_WP*d1*n12,f_t)
           this%p(i1)%debug(1)=delta_n
           this%p(i1)%debug(2)=f_n(2)*m1
           this%p(i1)%debug(3)=real(numiter, WP)
        end if
 ...
 subroutine collide_bons(this,dt,Gib,Nxib,Nyib,Nzib)
  ...
  subroutine advance(this,dt)
   ...
   !    ! Bounce-stick model from Bons (2017) "OSU Model"
   !    function get_osu_model_cor(p, Un) result(CoRn)
   !       implicit none
   !       type(part), intent(in) :: p
   !       real(WP), intent(in) :: Un
   !       real(WP) :: a, b, c
   !       real(WP) :: r, m, vol, KEn1, KEn2, Ecrit, ell, wel, wcrit, wmax, Acont, Uout, CoRn 
   ...
   !       if (wel.gt.wcrit) then
   !          wmax = ell - exp(log(ell - wcrit) - (KEn1 - Ecrit)/(this%sigma_y*vol))
   !          Acont = Acont*(a + b*(wmax/wcrit)**c) ! Contact area modified by adhesion
   !          KEn2 = Ecrit - this%gamma*Acont       ! Remove work of adhesion from available energy
   !       else
   !          Acont = Acont*(a + b*(wel/wcrit)**c)
   ...
   !       ! Compute normal coefficient of restitution
   !       CoRn = Uout/Un
   !    end function get_osu_model_cor
