...
module simulation
   ...
   real(WP), dimension(:,:,:), allocatable :: Lambda,EPSArr
   ...
   real(WP) :: Uvar,Vvar,Wvar,TKE,URMS,ell,sgsTKE,sgsEPS
...
contains
   ...
   subroutine simulation_run
      use sgsmodel_class, only: vreman, off, get_sgstke, get_sgseps
      implicit none
      integer :: ii
      print *,"[simulation_run] It's running"
      ...
      do while (.not.time%done())
         ...
         !if (use_crw) call lp%neighbor_corr()
         ...
         resU=fs%rho
         call sgs%get_visc(type=vreman,rho=resU,gradu=gradu)
         ...
         compute_stats: block 
               ...
               real(WP) :: mysgstke,myvisc,mysgsEps
               real(WP) :: tkebuf, epsbuf
               integer :: i,j,k,ierr
               ...
               do k=fs%cfg%kmin_,fs%cfg%kmax_
                  do j=fs%cfg%jmin_,fs%cfg%jmax_
                     do i=fs%cfg%imin_,fs%cfg%imax_
                        ! buffers for cell sgs tke and epsilon
                        tkebuf = get_sgstke(sgs%visc(i,j,k), sgs%delta(i,j,k), fs%rho)
                        epsbuf = get_sgseps(sgs%delta(i,j,k), tkebuf)

                        mysgstke=mysgstke + fs%cfg%vol(i,j,k)*tkebuf
                        mysgsEps=mysgsEps + fs%cfg%vol(i,j,k)*epsbuf
               ...
               EPS = 2.0_WP*visc*meanSR2/fs%rho !+ sgsEPS
