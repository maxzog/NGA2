...
module lpt_class
   ...
   contains
      ...
      procedure :: neighbor_corr                          !< Compute local spatial correlation for OU process
...
contains
      ...
      collision_force: block
         ...
         do i=1,this%np_
            ...
            if (delta_n.gt.0.0_WP) then
               ...
               this%p(i)%col=this%p(i)%col+f_n/m1
            end if
            ...
            if (this%cfg%nx.eq.1) this%p(i)%col(1)=0.0_WP
            if (this%cfg%ny.eq.1) this%p(i)%col(2)=0.0_WP
            if (this%cfg%nz.eq.1) this%p(i)%col(3)=0.0_WP
      ...
      do i=1,this%np_
         ...
         do while (dt_done.lt.dt)
            ...
            if (use_crw) then
               ...
               jjj = this%p(i)%ind(2)
               kkk = this%p(i)%ind(3)
               tke = get_sgstke(visc=sgs%visc(iii,jjj,kkk), delta=sgs%delta(iii,jjj,kkk), rho=rho(iii,jjj,kkk))
   ...
   subroutine get_rhs(this,U,V,W,rho,visc,T,p,acc,opt_dt,use_crw)
    ...
    !> Update diffusive component of OU process
    subroutine neighbor_corr(this)
      implicit none
      class(lpt), intent(inout) :: this
      integer, dimension(:,:,:), allocatable :: npic      !< Number of particle in cell
      integer, dimension(:,:,:,:), allocatable :: ipic    !< Index of particle in cell
               ...
               do jj=this%p(i)%ind(2)-no,this%p(i)%ind(2)+no
                  do ii=this%p(i)%ind(1)-no,this%p(i)%ind(1)+no
                     ...
                     do nn=1,npic(ii,jj,kk)
                        ...
                        if (this%p(i)%id.eq.this%p(i2)%id) cycle
