...
contains
   ...
   subroutine collide(this,dt,Gib,Nxib,Nyib,Nzib,Uib,Vib,Wib)
      implicit none
      ...
      real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(inout), optional :: Uib  !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
      real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(inout), optional :: Vib  !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
      real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(inout), optional :: Wib  !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
      ...
      check_G: block
         ...
         logical :: normal_present, velocity_present
         ! Do checks for normal and velocity vectors
         normal_present=present(Nxib).and.present(Nyib).and.present(Nzib)
         velocity_present=present(Uib).and.present(Vib).and.present(Wib)
         ! Kill if something is missing
         if (present(Gib).and..not.normal_present.or..not.velocity_present) then
      ...
      collision_force: block
         ...
         real(WP), dimension(3) :: r1,v1,w1,r2,v2,w2,v12,n12,f_n,t12,f_t,vbody
         ...
         collision: do i1=1,this%np_
            ...
            if (present(Gib)) then
               ...
               ! Get the velocity of the IB
               vbody(1)=this%cfg%get_scalar(pos=this%p(i1)%pos,i0=this%p(i1)%ind(1),j0=this%p(i1)%ind(2),k0=this%p(i1)%ind(3),S=Uib,bc='n')
               vbody(2)=this%cfg%get_scalar(pos=this%p(i1)%pos,i0=this%p(i1)%ind(1),j0=this%p(i1)%ind(2),k0=this%p(i1)%ind(3),S=Vib,bc='n')
               vbody(3)=this%cfg%get_scalar(pos=this%p(i1)%pos,i0=this%p(i1)%ind(1),j0=this%p(i1)%ind(2),k0=this%p(i1)%ind(3),S=Wib,bc='n')
               ! Compute relative velocity and overlap
               buf = sqrt(sum(n12*n12))+epsilon(1.0_WP)
               n12 = -n12/buf                                   ! Flip normal to point outward 
               rnv=dot_product(v1 - vbody,n12)                  ! Relative velocity (including body velocity)
               r_influ=min(2.0_WP*abs(rnv)*dt,0.2_WP*d1)        
               delta_n=min(0.5_WP*d1+r_influ-d12,this%clip_col*0.5_WP*d1)
               this%p(i1)%delta_n=0.0_WP
               if (delta_n.gt.0.0_WP) this%p(i1)%delta_n=1.0_WP
               ! Check if at discontinuity
               nearJump=abs(r1(1)).lt.d1
               ! Assess if there is collision
               if (delta_n.gt.0.0_WP.and..not.nearJump) then
                  ! Normal collision
                  k_n=m1*k_coeff_w
                  eta_n=m1*eta_coeff_w
                  f_n=-k_n*delta_n*n12-eta_n*rnv*n12
   ...
   subroutine inject(this,dt,avoid_overlap)
      ...
      if (avoid_overlap_) then
         ...
         inj_min(2)=this%inj_pos(2)-0.5_WP*this%cfg%yL-this%inj_dmax
         inj_max(2)=this%inj_pos(2)+0.5_WP*this%cfg%yL+this%inj_dmax
         inj_min(3)=this%inj_pos(2)-0.5_WP*this%cfg%zL-this%inj_dmax
         inj_max(3)=this%inj_pos(2)+0.5_WP*this%cfg%zL+this%inj_dmax
         do i=1,this%np_
   ...
   contains
      ...
      function get_position() result(pos)
        ...
        if (this%inj_D.gt.0.0_WP) then
           ...
           if (this%cfg%nz.eq.1) then
              pos(2)=random_uniform(lo=this%inj_pos(2)-0.5_WP*this%cfg%yL,hi=this%inj_pos(2)+0.5_WP*this%cfg%yL)
